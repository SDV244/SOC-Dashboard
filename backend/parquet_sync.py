...
_SELECT_COLS = """
    ...
    COALESCE(NULLIF(hostname, ''), NULLIF(computer_name, ''))
                                                               AS hostname,
    login_type,
    login_result,
    fidelity,
...
def convert_month(index: str, year: int, month: int) -> int:
    ...
    Path("/tmp/duckdb_tmp").mkdir(exist_ok=True)
    ...
    with ThreadPoolExecutor(max_workers=_DAY_WORKERS) as pool:
        for _day, rows, _err in (f.result() for f in as_completed(
            pool.submit(_convert_day_task, t) for t in tasks
        )):
            total_rows += rows
    ...
    for day_file in day_files:
        ...
        try:
            ...
            hosts = conn.execute(f"""
                SELECT coalesce(hostname, host, 'unknown'), count(*)
