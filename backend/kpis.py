...
def syslog_volume(
    ...
    if has_local_data(None, start, end):
        ...
        by_host = conn.execute(
            "SELECT coalesce(host, 'unknown'), count(*) FROM logs WHERE index IN ('syslog', 'wineventlog') AND ts BETWEEN ? AND ? GROUP BY 1 ORDER BY 2 DESC LIMIT ?",
            [start, end, limit],
        ).fetchall()
        ...
        return {
            "by_host": [{"host": r[0], "count": r[1]} for r in by_host],
