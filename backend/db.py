...
def get_oci_conn() -> duckdb.DuckDBPyConnection:
    ...
    conn.execute("SET threads=1; SET memory_limit='2GB';")
    setup_httpfs(conn)
...
def get_conn() -> duckdb.DuckDBPyConnection:
    ...
    if not hasattr(_local, "conn") or _local.conn is None:
        ...
        try:
            ...
            # DuckDB 1.5.2: WAL replay of ALTER TABLE TYPE fails.
            # Only delete WAL when connection fails — preserves valid WAL data.
            wal_path = Path(_db_path + ".wal")
            if wal_path.exists():
                wal_path.unlink()
            _local.conn = duckdb.connect(_db_path)
...
def _init_schema(conn: duckdb.DuckDBPyConnection) -> None:
    ...
    """)
    # Migrate: drop old auth_users if it has legacy integer id column
    try:
        cols = {r[0] for r in conn.execute("DESCRIBE auth_users").fetchall()}
        if "id" in cols:
    ...
    for col, typedef in [
        ...
        try:
            ...
            pass
    # Flush any schema migrations from WAL into the main DB file
    try:
