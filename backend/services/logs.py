...
def browse_logs(
    ...
    if not has_local_data(index, start, end):
    ...
    rows = conn.execute(
        f"""
        SELECT id, index, ts, severity, host, user_name, event_type, src_ip, dst_ip, raw
        FROM logs
        WHERE {where}
        ORDER BY ts DESC
        LIMIT ? OFFSET ?
...
def list_indexes():
    conn = get_conn()
    rows = conn.execute("SELECT DISTINCT index FROM logs ORDER BY index").fetchall()
...
def list_users():
    conn = get_conn()
    rows = conn.execute(
        "SELECT DISTINCT user_name FROM logs WHERE user_name IS NOT NULL ORDER BY user_name LIMIT 500"
    ).fetchall()
    return [r[0] for r in rows]
