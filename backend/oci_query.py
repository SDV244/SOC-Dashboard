...
def kpi_overview_oci(index: str, start: datetime, end: datetime) -> dict:
    ...
    stats = _daily_stats_range(index, start, end)
    if stats:
        result = {
            "total_events":      sum(r["total"] for r in stats),
            "threat_score_high": sum(r["threat_high"] for r in stats),
            "dga_detections":    sum(r["dga"] for r in stats),
            "tunneling_events":  sum(r["tunneling"] for r in stats),
            "_source": "daily_stats",
        }
        ...
        for day_row in stats:
            for t in day_row["top_threats"]:
                ...
                if dev not in totals:
                    totals[dev] = {"device": dev, "max_threat_score": t["max_score"], "count": 0}
            ...
            for day_row in stats:
                ...
                for entry in day_row["timeline_json"]:
                    timeline_rows.append({
                        "period": f"{date_str}T{int(entry['hour']):02d}:00:00",
                        "index": index,
                        "count": entry["count"],
                    })
    if any_stats:
        ...
        try:
            for src, tf, ts_expr in [
                ...
                hosts = conn.execute(f"""
                    SELECT coalesce(host,'unknown'), count(*)
                    FROM {src} WHERE {tf} GROUP BY 1 ORDER BY 2 DESC LIMIT {limit}
                """).fetchall()
                ...
                tl = conn.execute(f"""
                    ...
                    FROM {src} WHERE {tf} GROUP BY 1,2 ORDER BY 1
                """).fetchall()
                result["by_host"].extend({"host": r[0], "count": r[1]} for r in hosts)
...
def browse_logs_oci(
    ...
    # Gather all available Parquet paths across requested indexes
    all_paths: list[str] = []
    for idx in indexes:
    ...
    conn = get_oci_conn()
    try:
        src = _read_parquet(all_paths)
        tf = _ts_filter_parquet(start, end)

        conditions = [tf]

        if host:
            conditions.append(f"host ILIKE '%{host.replace(chr(39), '')}%'")
        if user_name:
            conditions.append(f"user_name ILIKE '%{user_name.replace(chr(39), '')}%'")
        if threat_score_min > 0:
            conditions.append(f"threat_score >= {int(threat_score_min)}")
        if is_dga:
            conditions.append(f"dga_flag = '{is_dga}'")
        if is_tunneling:
            conditions.append("tunneling > 0")
        if app_name:
            conditions.append(f"appid_name ILIKE '%{app_name.replace(chr(39), '')}%'")
        if src_country:
            conditions.append(f"country_code = '{src_country.replace(chr(39), '')}'")
        if domain:
            conditions.append(f"primary_domain ILIKE '%{domain.replace(chr(39), '')}%'")
        if search:
            s = search.replace("'", "")
            conditions.append(
        ...
        total = conn.execute(f"SELECT count(*) FROM {src} WHERE {where}").fetchone()[0]
        rows = conn.execute(f"""
            SELECT
                ts, src_ip, host, user_name, engid_name, appid_name,
                threat_score, dga_flag, tunneling, country_code,
                primary_domain, domain_reputation, msg_class, event_type,
                fidelity, dst_ip, severity
            FROM {src}
            WHERE {where}
            ORDER BY ts DESC
        ...
        return {"total": 0, "page": page, "page_size": page_size, "pages": 0, "items": [], "error": str(exc)[:300]}
...
def coverage_by_month(index: str, months: int = 3) -> list[dict]:
    ...
    while current <= end:
        ...
        if not is_converted(index, year, month):
            continue
        conn = get_oci_conn()
        try:
            src = f"'{parquet_path(index, year, month)}'"
            rows = conn.execute(f"""
                SELECT strftime(ts::DATE, '%Y-%m-%d') AS day, count(*)
                FROM parquet_scan({src})
                GROUP BY day ORDER BY day
            """).fetchall()
            results.extend({"day": r[0], "count": r[1]} for r in rows)
        except Exception:
            pass
