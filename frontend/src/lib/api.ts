...
export interface MlConfidenceData {
  avg_fidelity: number | null
  high_confidence_count: number
  bad_ip_reputation_count: number
...
export interface LoginStatsData {
  ...
  total_logins: number
  fail_ratio: number
  top_login_types: { type: string; count: number }[]
}
...
export const api = {
  ...
  availableMonths: () => get<{ year: number; month: number }[]>('/admin/available-months'),
  dataFreshness: () => get<{ last_data_date: string | null; next_sync_utc: string; sync_schedule: string }>('/admin/data-freshness'),

  mlConfidence: (p: Record<string, string | undefined>) => get<MlConfidenceData>('/kpis/ml/confidence', p),
  loginStats: (p: Record<string, string | undefined>) => get<LoginStatsData>('/kpis/logins/stats', p),
  topProcesses: (p: Record<string, string | number | undefined>) => get<ProcessEntry[]>('/kpis/processes/top', p),

  nlQuery: (query: string) =>
    post<{ filters: Partial<BrowseLogsParams>; query: string; error?: string }>('/logs/nl-query', { query }),
